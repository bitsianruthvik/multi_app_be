...
/** The short name somebody actually typed. */
 ...
 */
const shortOf = (record, def = null) => namedShort(record) ?? namedShort(def) ?? wordShort(record) ?? wordShort(def);
...
async function buildContext(db, companyId, master, draftValues = null) {
  ...
  // Where a temporary item sits: its owner line and order, and its BOM parent.
  let owner = null;
  let place = null;
  if (kind === 'temporary' && master.id != null && master.owner_order_line_id) {
...
async function pieceContext(db, companyId, draft) {
  ...
  return {
    ...
    test(cond) {
      if (cond.token_key !== 'kind') return { ok: false, weight: 0 };
      const values = cond.operator === 'in' ? cond.value.split(',').map((s) => s.trim()) : [cond.value.trim()];
      return { ok: values.includes(kind), weight: 1 };
    },
  };
}
...
registerEntity('production_piece', {
  ...
  async validateCondition(db, companyId, cond) {
    const values = cond.operator === 'in' ? cond.value.split(',').map((s) => s.trim()).filter(Boolean) : [cond.value.trim()];
    return values.every((v) => ['catalog', 'temporary'].includes(v)) ? null : 'Kind is catalog or temporary.';
  },
  async loadContext(db, companyId, entityId) {
    const [[p]] = await db.query(
      `SELECT pi.item_id, pi.piece_no, r.order_id, l.line_no, parent.code AS parent_code
    ...
    return pieceContext(db, companyId, { itemId: p.item_id, orderId: p.order_id, lineNo: p.line_no, parentCode: p.parent_code, pieceNo: p.piece_no });
  },
  async draftContext(db, companyId, draft) {
