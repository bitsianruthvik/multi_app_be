...
/**
 * The saved plan as a workbook, or a fresh proposal when nothing is saved yet —
 * because a person who opens the sheet on a line that has never been nested
 * wants something to edit, not an empty file. `plan` lets a caller hand in the
 * proposal it is already showing, so the sheet matches the screen exactly.
 */
export async function exportNestingSheet(db, companyId, orderLineId, { plan = null, pack } = {}) {
  const model = plan ?? await nestingForSheet(db, companyId, orderLineId, pack);
  const rows = [];
    ...
    for (const n of g.nests) {
      for (const p of n.pieces) {
        rows.push({
          ...
          plateSize: `${fmt(n.length)} × ${fmt(n.width)}`,
          thickness: n.thickness,
          grade: n.grade ?? '',
          material: n.material ?? '',
          cutPlateCode: p.cutPlateCode,
          cutPlateName: p.cutPlateName ?? '',
          seqNo: p.seqNo, rowNo: p.rowNo, posNo: p.posNo,
          x: p.x, y: p.y, length: p.length, width: p.width,
  ...
  ws.columns = SHEET_COLUMNS.map((c) => ({ width: c.width }));
  const head = ws.addRow(SHEET_COLUMNS.map((c) => c.header));
  head.font = { bold: true };
  head.alignment = { vertical: 'middle', wrapText: true };
  for (const r of rows) {
    const line = ws.addRow(SHEET_COLUMNS.map((c) => r[c.key]));
    SHEET_COLUMNS.forEach((c, i) => {
      if (c.locked) line.getCell(i + 1).fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FFF2F2F2' } };
    });
  }
  ...
  notes.getRow(3).font = { bold: true, color: { argb: 'FFB00020' } };
...
async function nestingForSheet(db, companyId, orderLineId, pack) {
...
/**
 * The sheet back. UPLOADING IT IS ACCEPTING IT — the file is turned into a plan
 * and handed straight to acceptNesting, which verifies it against the database
 * exactly as it verifies one from the screen. The sheet says so on its own face
 * because in fab it surprised people.
 */
export async function importNestingSheet(db, c, orderLineId, input = {}) {
  if (blank(input.fileBase64)) throw invalid('NO_FILE', 'Send the workbook as `fileBase64`.');
  const buffer = Buffer.from(String(input.fileBase64).replace(/^data:[^,]+,/, ''), 'base64');
  const ExcelJS = (await import('exceljs')).default;
  const wb = new ExcelJS.Workbook();
  try { await wb.xlsx.load(buffer); } catch { throw invalid('BAD_FILE', 'That file could not be opened as a workbook.'); }
  ...
  const header = [];
  ws.getRow(1).eachCell({ includeEmpty: true }, (cell, i) => { header[i - 1] = HEADER_TO_KEY.get(normaliseHeader(cell.value)) ?? null; });
  if (!header.includes('lot') || !header.includes('cutPlateCode')) {
    throw invalid('WRONG_SHEET', 'That sheet has no Lot and Cut Plate Code columns, so it is not a nesting layout. Download the sheet and edit that copy.');
  }

  const problems = [];
  const rows = [];
  ws.eachRow({ includeEmpty: false }, (row, n) => {
    if (n === 1 || rows.length >= MAX_SHEET_ROWS) return;
    const r = {};
    row.eachCell({ includeEmpty: true }, (cell, i) => {
      const key = header[i - 1];
      if (!key) return;
      const v = cell.value;
      r[key] = v && typeof v === 'object' ? (v.text ?? v.result ?? v.richText?.map((t) => t.text).join('') ?? null) : v;
    });
    if (blank(r.lot) && blank(r.cutPlateCode)) return;
    r.__row = n;
    rows.push(r);
  });
  if (!rows.length) throw invalid('EMPTY_SHEET', 'That sheet has no rows on it. The sheet IS the whole plan, so an empty one is not "no change" — it is a plan with no plates, which cannot be accepted.');

  // Codes, not ids, because a person types codes. Resolved against this
  // company's own records, so a code from another tenant simply is not found.
  const codes = [...new Set(rows.flatMap((r) => [r.plateCode, r.cutPlateCode]).filter((x) => !blank(x)).map((x) => String(x).trim()))];
  ...
  const resolve = (code, what, at) => {
    const id = idByCode.get(String(code ?? '').trim().toUpperCase());
    if (!id) problems.push(`Row ${at}: there is no ${what} with the code ${blank(code) ? '(blank)' : code}.`);
    return id ?? null;
  };
  ...
  for (const r of rows) {
    const at = r.__row;
    const lotKey = blank(r.lot) ? `(row ${at})` : String(r.lot).trim();
    if (!lots.has(lotKey)) lots.set(lotKey, { lotNo: lotKey, plateItemId: null, plateCode: null, pieces: [] });
    const lot = lots.get(lotKey);
    const plateId = resolve(r.plateCode, 'catalog plate', at);
    if (plateId != null) {
      if (lot.plateItemId != null && lot.plateItemId !== plateId) {
        problems.push(`Row ${at}: lot ${lotKey} is given two different plates (${lot.plateCode} and ${r.plateCode}). A lot is ONE physical plate — put the second one on a lot of its own.`);
      } else { lot.plateItemId = plateId; lot.plateCode = String(r.plateCode).trim(); }
    }
    const cutPlateId = resolve(r.cutPlateCode, 'cut plate', at);
    const rot = String(r.rotated ?? '').trim().toLowerCase();
    lot.pieces.push({
      cutPlateId,
      seqNo: pos(r.seqNo) ?? 1,
      rowNo: pos(r.rowNo) ?? 1,
      ...
      length: num(r.length),
      width: num(r.width),
      rotated: ['yes', 'y', 'true', '1'].includes(rot),
    });
  }
  assertNoProblems(problems, 'That sheet could not be read.');

  const out = await acceptNesting(db, c, orderLineId, { nests: [...lots.values()] });
  return { ...out, source: 'sheet', sheetRows: rows.length };
}
