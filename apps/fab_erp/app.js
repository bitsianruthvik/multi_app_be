...
import catalogDetailRoutes    from './routes/catalogDetail.js';
...
export default {
  ...
  register(server) {
    ...
    // Item page reads (where-used / stock / purchases) + taxonomy counts.
    // Read-only; a NEW router for the same concurrent-edit reason as above.
    server.use('/api/:companySlug/fab_erp', catalogDetailRoutes);
