...
// EU-2: resources whose `code` is server-autogenerated on insert; client value is ignored.
const AUTOGEN_CODE_RESOURCES = { fabErpCustomer: 'customer', fabErpSupplier: 'supplier' };
...
export async function mutate(req, res) {
  ...
  try {
    if (op === 'insert') {
      ...
      if (AUTOGEN_CODE_RESOURCES[resource]) {
        row.code = await generateCode(companyId, AUTOGEN_CODE_RESOURCES[resource], {});
      }
