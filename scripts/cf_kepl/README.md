...
| `update-cf-prod.sh` | the runner that sequences all of the above against production, in the order they depend on each other: schema, shop floor, operations, flows, wiring, reconcile, re-code, verify. Refuses without `--yes`, checks every file it needs exists BEFORE it touches production, and reads credentials from `TM/.env.tidb` — deliberately outside this repo, never committed (`CF_ENV_FILE` to override). Run it from anywhere |
