 ...
 *
 * CUT PLATES ARE IN SCOPE TOO. A cut plate (material_form='blank') is minted
 * per order and lives in a per-order group under Cut Plates; with the order
 * gone it describes nothing. Its catalog row, its field values, any stock
 * piece of it, and the Cut Plates groups are soft-deleted. The Cut Plates
 * CATEGORY stays — it is a system row the next order's nesting reuses.
 *
 * DRY RUN BY DEFAULT. Pass --apply to write.
 *
 *   node scripts/wipe-orders.mjs                   # report only, TiDB
 *   node scripts/wipe-orders.mjs --apply           # do it
 *   node scripts/wipe-orders.mjs --company=30005   # one tenant only
 *   node scripts/wipe-orders.mjs --local           # local MySQL instead of TiDB
 */
...
const conn = LOCAL
  ? await mysql.createConnection({ host: 'localhost', port: 3306, user: 'root', password: '1234', database: 'sqldb' })
...
console.log(`${APPLY ? 'APPLY' : 'DRY RUN'} — ${LOCAL ? 'local MySQL' : 'TiDB'} — ${COMPANY ? `company ${COMPANY}` : 'ALL companies'}\n`);

const CO = COMPANY ? ' AND company_id = ?' : '';
const coArgs = COMPANY ? [COMPANY] : [];

const [byCompany] = await conn.query(
  `SELECT o.company_id, c.slug, o.order_type, COUNT(*) n FROM fab_orders o
     LEFT JOIN companies c ON c.id = o.company_id
    WHERE o.deleted_at IS NULL${COMPANY ? ' AND o.company_id = ?' : ''}
    GROUP BY o.company_id, c.slug, o.order_type ORDER BY o.company_id`, coArgs);
console.log('live orders by company:');
console.table(byCompany);

/** Every live order, and the items and tasks under them. */
const [orders] = await conn.query(`SELECT id FROM fab_orders WHERE deleted_at IS NULL${CO}`, coArgs);
const [items] = await conn.query(`SELECT id FROM fab_items WHERE deleted_at IS NULL${CO}`, coArgs);
const [tasks] = await conn.query(`SELECT id FROM fab_project_tasks WHERE deleted_at IS NULL${CO}`, coArgs);
/** Cut plates: every blank catalog row, and the per-order groups under the system Cut Plates category. */
const [cutPlates] = await conn.query(
  `SELECT id FROM fab_item_catalog WHERE material_form = 'blank' AND deleted_at IS NULL${CO}`, coArgs);
const [cutGroups] = await conn.query(
  `SELECT g.id FROM fab_item_groups g JOIN fab_item_categories k ON k.id = g.category_id
    WHERE k.code = 'cut' AND k.is_system = 1 AND g.deleted_at IS NULL${COMPANY ? ' AND g.company_id = ?' : ''}`, coArgs);
const orderIds = orders.map((r) => r.id);
const itemIds = items.map((r) => r.id);
const taskIds = tasks.map((r) => r.id);
const cutPlateIds = cutPlates.map((r) => r.id);
const cutGroupIds = cutGroups.map((r) => r.id);

if (!orderIds.length && !cutPlateIds.length && !cutGroupIds.length) { console.log('No live orders or cut plates. Nothing to do.'); await conn.end(); process.exit(0); }
console.log(`orders=${orderIds.length}  items=${itemIds.length}  tasks=${taskIds.length}  cutPlates=${cutPlateIds.length}  cutPlateGroups=${cutGroupIds.length}\n`);
...
const PLAN = [
  ...
  ['fab_task_time_overrides', 'order_id', orderIds],
  ...
  ['fab_nesting_runs', 'order_id', orderIds],
  ['fab_month_marks', 'order_id', orderIds],
  ['fab_month_marks', 'production_order_id', orderIds],
  ['fab_item_demand', 'order_id', orderIds],
  ['fab_order_pieces', 'order_id', orderIds],
  ['fab_order_structure_revisions', 'order_id', orderIds],
  ...
  // cut plates: any stock of them, then the catalog rows, then their groups
  ['fab_stock_pieces', 'catalog_item_id', cutPlateIds],
  ['fab_item_catalog', 'id', cutPlateIds],
  ['fab_item_subgroups', 'group_id', cutGroupIds],
  ['fab_item_groups', 'id', cutGroupIds],
];
...
try {
  ...
  // A cut plate's own field values (scope catalog_item).
  let cfv = 0;
  for (const part of chunks(cutPlateIds)) {
    const where = `WHERE scope = 'catalog_item' AND scope_id IN (${part.map(() => '?').join(',')}) AND deleted_at IS NULL`;
    if (APPLY) {
      const [r] = await conn.query(`UPDATE fab_field_values SET deleted_at = NOW() ${where}`, part);
      cfv += r.affectedRows;
    } else {
  ...
  report.push({ table: 'fab_field_values (cut plates)', via: "scope='catalog_item'", rows: cfv });
...
const [[left]] = await conn.query(`SELECT
  ...
  (SELECT COUNT(*) FROM fab_item_catalog WHERE deleted_at IS NULL AND material_form = 'blank') cut_plates,
